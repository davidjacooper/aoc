...
fn main()
{
    ...
    match std::fs::read_to_string(&file)
    {
        ...
        Ok(content) =>
        {
            ...
            cosmic_expansion_part2(&content);
            println!("---\ntime: {:?}", Instant::now().duration_since(start));
        }
    }
}

fn cosmic_expansion_part2(content: &str)
{
    ...
    for g1 in 0..(galaxies.len() - 1)
    {
        ...
        for g2 in g1..galaxies.len()
        {
            ...
            let distance = (expanded_rows[i2] as i64 - expanded_rows[i1] as i64).abs() +
                           (expanded_cols[j2] as i64 - expanded_cols[j1] as i64).abs();
            sum += distance;
        }
    }
