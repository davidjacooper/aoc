...
fn clumsy_crucible_part2(content: &str)
{
    ...
    loop
    {
        ...
        if node.i == height - 1 && node.j == width - 1 && node.straight_dist >= 4 { break }
          ...
          steps: &mut HashMap<Node,Node>)
{
    ...
    if from_dir == direction.opposite() ||
        (from_dir != direction && straight_dist > 0 && straight_dist < 4) ||
        (from_dir == direction && straight_dist == 10) { return; }
