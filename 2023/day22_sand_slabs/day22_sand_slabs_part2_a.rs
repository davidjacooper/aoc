...
/*
 * Here, we re-do the entire simulation for each brick, each time pretending that the given brick
 * doesn't exist, and count the number of other bricks that end up in different places, compared
 * with the base case.
 */
...
fn sand_slabs_part1(content: &str) -> Result<(),&str>
{
    ...
    for missing_brick_i in 0..bricks.len()
    {
        ...
        let n_fall =
            (0..bricks.len())
            .filter(|j| *j != missing_brick_i && normal_bricks[*j] != test_bricks[*j])
            .count();
        sum += n_fall;
