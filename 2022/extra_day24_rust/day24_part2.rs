...
fn blizzard_basin_part2(mut content: &str)
{
    ...
    run(&mut time, start, end, &mut grid, &mut blizzards);
    run(&mut time, end, start, &mut grid, &mut blizzards);
    run(&mut time, start, end, &mut grid, &mut blizzards);
}
       ...
       blizzards: &mut Vec<Blizzard>)
{
    ...
    while !finished
    {
        ...
        for (ex_i, ex_j) in expeditions.drain()
        {
            ...
            if ex_i < (height - 1) && grid[ex_i + 1][ex_j] == 0
            {
                next_expeditions.insert((ex_i + 1, ex_j));
            }
