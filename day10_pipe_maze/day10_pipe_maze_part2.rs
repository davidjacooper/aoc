...
fn pipe_maze_part1(content: &str)
{
    ...
    let start_up = "|7F".contains(grid[start_i - 1][start_j]);
    let start_down = "|JL".contains(grid[start_i + 1][start_j]);

    use Dir::*;
    let (mut i, mut j, mut dir) =
        if      start_up   { (start_i - 1, start_j, Up) }
        else if start_down { (start_i + 1, start_j, Down) }
    ...
    let mut enclosed_set = HashSet::<(usize, usize)>::new();
        ...
        for (j, ch) in row.iter().enumerate()
        {
            if path_set.contains(&(i, j))
            {
                in_out = match (ch, &in_out)
                {
                    ...
                    ('S', Outside) if start_up && start_down => Inside,
                    ('S', Inside)  if start_up && start_down => Outside,
                    ('S', Outside) if start_up               => BottomLine,
                    ('S', Outside) if start_down             => TopLine,
                    ('S', Inside)  if start_up               => TopLine,
                    ('S', Inside)  if start_down             => BottomLine,
    ...
    for (i, row) in grid.iter().enumerate()
    {
        for (j, ch) in row.iter().enumerate()
        {
            ...
            else if enclosed_set.contains(&(i, j))
            {
                print!("\x1b[45;1m{}\x1b[m", ch);
            }
            else
            {
                print!("{ch}");
